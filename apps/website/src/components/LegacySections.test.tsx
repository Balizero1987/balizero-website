...
describe("legacy section journeys", () => {
  it("keeps review attribution on its public source without inventing testimonials", () => {
    render(<Reviews />);
    expect(screen.getByRole("link", { name: /Google Reviews/ })).toHaveAttribute("href", "https://maps.app.goo.gl/whiMUTNchcDR5naz8");
    expect(screen.queryByRole("blockquote")).not.toBeInTheDocument();
    expect(screen.getByText("Opens the public Google profile")).toBeInTheDocument();
  });
  ...
  it("makes About reachable locally and uses homepage anchors from inner pages", () => {
    ...
    expect(screen.getByRole("link", { name: "Our tools" })).toHaveAttribute("href", "/#tools");
    expect(screen.getByRole("link", { name: "E-VOA" })).toHaveAttribute("href", "/#evoa");
  ...
  it("provides the company story, existing founders and next steps", () => {
    ...
    expect(screen.getByRole("heading", { name: "A clearer way to get started." })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Meet our team/ })).toHaveAttribute("href", "/team");
    expect(screen.getByRole("link", { name: /Read the Journal/ })).toHaveAttribute("href", "/journal");
  });
  ...
  it("opens an honest assistant state with usable contacts and returns focus on close", async () => {
    ...
    const close = vi.fn(() => { panel.removeAttribute("open"); fireEvent(panel, new Event("close")); });
    ...
    expect(screen.getByText("Chat unavailable in this preview")).toBeInTheDocument();
    expect(screen.queryByRole("textbox")).not.toBeInTheDocument();
    expect(screen.getByRole("link", { name: /Continue on WhatsApp/ })).toHaveAttribute("href", expect.stringContaining("https://wa.me/628213454721?text="));
