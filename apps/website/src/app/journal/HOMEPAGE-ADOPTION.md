...
The coordinator-owned homepage Journal should keep its compact editorial role while delegating the complete inventory to `/news`.

Recommended adoption after this lane is integrated:

1. Keep the “Explore the Journal” destination on the local `/news` index.
...
4. Do not introduce local `/news/[slug]` links until authorized article documents exist. Source-only records must continue to open their verified original destinations.
