...
import {
  articleLocales,
  isArticleLocale,
  loadAuthoredTranslations,
} from "../../../lib/server/article-translations";
...
export async function generateMetadata({
  ...
  if (!isArticleLocale(lang))
    return {
      title: "Bali Zero Journal",
  ...
  const translated =
    lang === "en"
      ? undefined
      : (await loadAuthoredTranslations(category, slug))[lang];
  const article = translated ?? result.article;
  return {
    title: article.metadata.title,
    description: article.standfirst,
    alternates: { canonical: article.metadata.sourceUrl },
  };
}
...
export default async function ArticlePage({ params, searchParams }: Props) {
  ...
  const translations: Awaited<ReturnType<typeof loadAuthoredTranslations>> =
    result.status === "ready"
      ? await loadAuthoredTranslations(category, slug)
      : {};
  const translated = translations[requestedLocale];
  const locale = translated ? requestedLocale : "en";
  const article =
    translated ?? (result.status === "ready" ? result.article : null);
  const href = `/${category}/${slug}`;
  const suffix = requestedLocale === "en" ? "" : `?lang=${requestedLocale}`;
  return (
    <SiteShell mainId="article-content" className={styles.indexTheme}>
      <main id="article-content" tabIndex={-1}>
        {article ? (
          <>
            <nav
              aria-label="Breadcrumb"
              className={`${styles.breadcrumb} ${readerUi.breadcrumb}`}
            >
            ...
            <nav className={localeUi.languages} aria-label="Article language">
              <span>Read in</span>
              {Object.entries(articleLocales)
                .filter(
                  ([key]) =>
                    key === "en" ||
                    translations[key as keyof typeof articleLocales],
                )
                .map(([key, label]) => (
                  <a
                    key={key}
                    href={href + (key === "en" ? "" : `?lang=${key}`)}
                    lang={key}
                    hrefLang={key}
            ...
            {requestedLocale !== "en" && !translated ? (
              <p role="status" className={localeUi.notice}>
                The {articleLocales[requestedLocale]} edition is not available
                here. You are reading the English article.
              </p>
            ) : null}
            <ArticleTemplate article={article} locale={locale} />
            <section className={`${styles.readNext} ${readerUi.readNext}`}>
              <h2>Continue reading</h2>
              <a href={`/news?category=${category}`}>
                More in {article.metadata.category}{" "}
                <span aria-hidden="true">↗</span>
              </a>
            </section>
          </>
          ...
          <section className={styles.readNext}>
            ...
            <p role="status">
              We could not load the article. Please try again in a moment.
            </p>
            <p>
              <a href={href + suffix}>Try again</a>
            </p>
            <p>
              <a href={`/legacy${href}${suffix}`}>
